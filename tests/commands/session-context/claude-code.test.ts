...
describe("claude-code action handler", () => {
  ...
  let readSpy: Mock<typeof sessionContextHelpers.readClaudeCodeSession>;
  let listSpy: Mock<typeof sessionContextHelpers.listClaudeCodeSessions>;
  ...
  beforeEach(() => {
    ...
    listSpy = spyOn(sessionContextHelpers, "listClaudeCodeSessions");
  ...
  test("prints JSON on successful result", async () => {
    ...
    // (not the real ClaudeSessionSummary) is enough to exercise passthrough.
    // oxlint-disable-next-line typescript/no-unsafe-type-assertion
    readSpy.mockResolvedValue({
      ok: true,
      data: { entries: [{ role: "user", content: "hello" }], total: 1 },
    } as unknown as Awaited<
      ReturnType<typeof sessionContextHelpers.readClaudeCodeSession>
    >);
