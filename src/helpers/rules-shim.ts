 ...
 * Markdown frontmatter in one object. For \`.yml\`/\`.yaml\`,
 * \`frontmatter\` is \`null\` and \`content\` is the parsed document;
 * otherwise \`frontmatter\` is the parsed leading block and \`content\` the
 * trimmed body text.
 */
...
declare interface RuleContext {
   ...
   * \`ReadYamlResult\`. Dispatch is extension-based, so a multi-document
   * stream's \`---\` separators are never misread as frontmatter.
   * \`frontmatter\` is \`null\` when a file has none — one null test,
   * mirroring \`fileAtBase\` — and \`{}\` when the block is empty.
   *
