...
**Why:** After the inquirer v14 `"list"` → `"select"` crash, both a draft ADR `.rules.ts` allowlist and a bun test scanning source files were rejected by the user: an untestable-in-CI check isn't a real test, and a per-file syntax check isn't ADR governance. It landed instead as a custom oxlint JS plugin rule (`archgate/valid-inquirer-prompt-type` in `.archgate/lint/oxlint.ts`) — real AST access, runs in the existing `bun run lint` gate.

**Reinforced (rule-scanner RCE):** I proposed an ADR + companion rules for the `.rules.ts` sandbox boundary; the user cut the rules — _"your proposal for rules is actually only testing the implementation and not the behaviour"_ — and took the ADR alone (`rules: false`, ARCH-024). The proposed rules ("scanner defines `ALLOWED_MODULES`", "loader scans before `import()`") asserted implementation _shape_, and the second **would have passed for the entire lifetime of the vulnerability** — the loader did scan, in the right order; the scan simply didn't work. A structural check cannot distinguish a boundary from the appearance of one. Behaviour (`a rule file cannot reach child_process`) is provable only by executing an attack: `tests/engine/rule-scanner-escapes.test.ts`.
...
- **Before proposing a companion `.rules.ts`, ask what it would assert if the implementation were subtly broken.** If the honest answer is "it would still pass," it tests shape, not behaviour — write a test and ship the ADR as `rules: false`. An ADR with no rules is a complete outcome; a tautological rule is worse than none, because a green check reads as evidence.
...
- **A check that describes the CLI's OWN behaviour belongs in the CLI, not in a dogfooded ADR.** Reinforced 2026-07-25: I shipped the ADR briefing-budget check as an ADR (GEN-005, since deleted) plus companion rules; the user cut it — _"this should be baked in archgate cli. Not an adr plus rules."_ Two symptoms should have caught it first: the rule read a constant (`DEFAULT_MAX_SECTION_CHARS`) out of `src/engine/context.ts` with a hardcoded fallback for every other project, and it reimplemented `extractAdrSections` because ARCH-024 forbids a rule file importing engine code. **If a rule needs to reach into the CLI's internals, or duplicate them, it is describing product behaviour that every archgate user shares — ship it as a built-in.** Built-in diagnostics attach to `CheckResult`/`ReportSummary` alongside `suppressionWarnings` and render in console, CI annotations, and JSON. Test: ask whether a consumer project would want this check too. If yes, it is not this project's governance.
...
- **oxlint jsPlugins have full module resolution and top-level await** — a plugin can `await import("inquirer")` at load and derive its allowlist from the installed dependency's runtime state (e.g. `Object.keys(inquirer.prompt.prompts)`). Prefer this over hardcoded allowlists: the rule self-updates on dependency bumps and stale call sites fail lint in the bump PR itself. Guard with a loud throw if the upstream API shape changes, so the rule can never silently disable itself.
