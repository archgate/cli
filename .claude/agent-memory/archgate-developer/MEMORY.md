...
- **`archgate review-context` omits every ADR's `decision`/`dosAndDonts` prose by default** — the `@reviewer` skill's own instructions describe the briefing as containing "only the Decision and Do's and Don'ts sections," but the CLI's actual default (confirmed via `--help`) leaves both fields off; only `--verbose` includes them. Without it, per-domain sub-agent prompts built from the response carry `id`/`title`/`domain` and nothing to actually check code against. Always pass `--verbose` (per-domain, via `--domain <name>`, to keep each call small) when gathering context for the reviewer's sub-agent prompts.
