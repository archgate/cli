...
- **`archgate review-context`'s `--base` (auto-detect or explicit `origin/main`) diffs against the local `main`/`origin/main` ref, which can be stale-but-tree-identical after a squash merge** — same content, different commit hash, so it inflates `allChangedFiles` with every file from the last merged PR. Before trusting its output, `git fetch origin main:main`; if `git diff origin/main HEAD --stat` is empty the trees already match and `git reset origin/main` (never `--hard`) safely realigns the branch pointer without touching uncommitted work. In a git worktree, `fetch origin main:main` is refused (`main` is checked out in the primary tree) — run plain `git fetch origin` and pass `--base origin/main` explicitly instead.
