...
- **PowerShell `Select-String` is case-insensitive by default, which makes mojibake scans lie.** Searching for `Ã`/`Â` to find double-encoded UTF-8 also matches legitimate `ã`/`â` in pt-br prose, so every accented line looks corrupted. Pass `-CaseSensitive`. Do NOT reach for a byte-length-vs-character-length comparison as the alternative — every legitimate accented character is multi-byte in UTF-8, so that flags correct prose too; scan for the specific corruption sequences instead.
- **Verify translations yourself rather than delegating to a review sub-agent.** Per [[project_rules_engine_internals]], review agents misquote non-English prose they have read and will "correct" text that is already right — the failure mode is worst exactly where the reviewer cannot sanity-check by meaning.
