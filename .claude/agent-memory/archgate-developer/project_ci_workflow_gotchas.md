...
- **`actions/create-github-app-token` needs `permission-*` inputs** (e.g. `permission-contents: write`) — without them the token inherits every permission of the app installation (zizmor `github-app` finding). Scope per workflow: update-lock/update-llms need only `permission-contents: write`; release.yml jobs need contents+pull-requests+issues write for simple-release-action.
