...
The URL carries no credentials. `archgate credential` is the credential helper; `archgate login` registers it for `plugins.archgate.dev`. Git asks it on each request and receives a freshly renewed access token.
...
Replace `<token>` with a current access token. `archgate credential get` emits a git credential response rather than a bare token, so take the `password=` line:

```bash
printf 'protocol=https\nhost=plugins.archgate.dev\n\n' | archgate credential get | sed -n 's/^password=//p'
```

Access tokens are short-lived, so a value pasted here stops working within the hour — prefer the marketplace setup above, where git asks the helper on every request.
