...
Sign in to obtain plugin access:

```bash
archgate login
```

This starts an OAuth 2.0 Device Authorization Grant against the Archgate identity provider. The CLI displays a one-time code and URL -- open the URL in your browser, enter the code, and authorize. Once complete, credentials are stored securely in your OS credential manager via `git credential approve`.
