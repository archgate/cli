...
Two details matter here: the upload step needs `if: always()`, because `archgate check` exits 1 on violations and the findings must still reach the Security tab when there are findings; and the job needs the `security-events: write` permission, or the upload is rejected.
