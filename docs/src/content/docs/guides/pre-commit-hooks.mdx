...
When a check fails, the commit is blocked. Violations are printed to stdout with file paths and line numbers so you can locate and fix them immediately.
...
| `--output json`   | Output results as JSON -- useful for piping to other tools or custom reporting scripts                                     |
| `--adr <id>`      | Only check rules from a specific ADR -- useful for isolating a single rule during debugging                                |
| `--output github` | Output GitHub Actions annotations -- use this in CI workflows instead of pre-commit hooks                                  |
