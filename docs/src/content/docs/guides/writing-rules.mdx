...
Rules are TypeScript functions that lint your codebase against your team's decisions. They live in companion `.rules.ts` files next to ADR markdown files and run when you execute `archgate check`.
...
- `{ comments: true }` attaches a structured `comments` array to the tree (`typescript`/`javascript`/`python`), so comment-governance rules can work against comment tokens instead of regex.
...
Read a file's source at the **base git revision** -- the merge base of the `--base` ref and `HEAD`. Returns `null` when no base is resolved (no `--base`) or the file did not exist at the base (an added file), so a single null check covers both. For a structural comparison, prefer `ctx.ast(path, language, { rev: "base" })`.
...
`ctx.ast(path, language, { comments: true })` attaches a `comments` array to the tree so comment-policy rules work against structured tokens (`type`, `value`, `loc`) instead of line-by-line regex. Comment positions are accurate against the original source even for TypeScript. Supported for `typescript`, `javascript`, and `python`.
