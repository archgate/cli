...
- **Antigravity** — the `agy` CLI writes each conversation as JSONL under `~/.gemini/antigravity-cli/brain/<id>/.system_generated/logs/`, and records the workspace it belongs to in a sibling SQLite database. Only the CLI is readable: the Antigravity IDE keeps a separate store whose transcripts are encrypted at rest.
