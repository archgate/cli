...
**opencode:** The CLI downloads an authenticated tarball of agent files from the Archgate plugins service and extracts it into the user-scope opencode agents directory (`~/.config/opencode/agents/` on POSIX, `%APPDATA%\opencode\agents\` on Windows). No files are written to the project tree. See the [opencode integration guide](/guides/opencode-integration/) for details.
