...
Confirm you are signed in with `archgate login status`. If the refresh token has expired or been revoked, `archgate credential get` returns no credentials and reports the error rather than staying silent. Run `archgate login` again.
