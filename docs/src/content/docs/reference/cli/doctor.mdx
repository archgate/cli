...
The **Git helper** line reports `OK` when the entry names the running archgate binary and is preceded by the reset entry that keeps other helpers from answering first. `MISSING` means a platform session is stored but the entry is absent; `STALE` means it names another archgate binary, typically after an upgrade or a reinstall; `SHARED` means the reset entry is gone and another helper may answer first; `NO SESSION` means the entry exists but nothing is stored for it to serve. Each state is followed by the command that repairs it. A plugin `git clone` that prompts for a password usually traces back to one of these.
