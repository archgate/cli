...
The `if: always()` is required: `archgate check` exits 1 when it finds violations, which would otherwise skip the upload step exactly when there are findings to report. The job also needs the `security-events: write` permission for the upload to succeed.
...
Advisory diagnostics are corpus-wide, not rule-scoped: they are collected and enforced even when no `rules: true` ADR exists, so a prose-only ADR corpus still fails `--strict` on a briefing overrun or an unparseable ADR file.
