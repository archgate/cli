...
These are **advisory and never affect `pass`**. An ADR whose section cannot shrink without dropping a normative clause — an enumerated identifier list, an ordered guardrail, an exemption — is expected to exceed the cap. Where that happens, record why in the ADR's own Compliance section and order the section so its most normative content precedes the cut, since truncation always removes the tail.

To reduce an overflow: move rationale into Context or Consequences, which are never briefed and therefore never capped; drop historical narration; and merge duplicate rules.
