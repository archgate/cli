...
- **Command name** and **which flags were used** (e.g., `check --output json`; only flag presence, never flag values)
...
- **No flag values**: we record that `--output json` was used, not what the JSON output contained
