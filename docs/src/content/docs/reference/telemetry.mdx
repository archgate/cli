...
- **No network activity**: no API keys or tokens, and no URLs — with one exception: the remote URL of a repository confirmed public by its host, in the one-time `project_initialized` event (see [Repo identity](#repo-identity))
