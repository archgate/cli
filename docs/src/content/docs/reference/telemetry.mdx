...
- Private repositories (API probe returns 404 or `private: true`)
...
**Opt-out** (for public repos where you'd still rather stay anonymous):

```bash
# Per-invocation flag
archgate init --no-share-repo-identity

# Or via environment variable — applies to every invocation
export ARCHGATE_SHARE_REPO_IDENTITY=0
archgate init
```

Accepted opt-out env values: `0`, `false`, `no`, `off` (case-insensitive).
