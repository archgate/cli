...
- **Scan transitives (the default here)**: `ctx.glob("node_modules/{*,@*/*}/package.json")` already walks every installed package — direct and transitive, including scoped packages via the `@*/*` branch.
- **Check only production deps**: Read the root `package.json` with `ctx.readJSON("package.json")` and filter `pkgFiles` down to names present in its `dependencies` field if you only want to gate bundled/shipped code, not `devDependencies`.
