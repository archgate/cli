...
description: Importer kuraterte Architecture Decision Records fra Archgate-registeret eller et hvilket som helst git-repositorium. Start med utprovde konvensjoner i stedet for en blank side.
---
...
- Valgfrie tilhorende `.rules.ts`-filer som haandhever hver beslutning automatisk

Pakker lar deg starte et prosjekt med utprovde arkitekturkonvensjoner i stedet for aa skrive alt fra bunnen av.
...
Dette kloner registeret, kopierer ADR-ene til `.archgate/adrs/`-katalogen din, og tilordner ID-er paa nytt for aa passe prosjektets nummereringsskjema.

### Laase til en versjon

Legg til `@<ref>` for aa laase til en spesifikk git-tag eller gren:
...
Bare den ene ADR-en (og dens tilhorende regelfil, hvis den finnes) vil bli importert.
...
For ikke-GitHub-repositorier eller naar du trenger full kontroll, send en fullstendig URL:
...
## Forhaandsvis med `--dry-run`

Se hva som ville blitt importert uten aa skrive noe:
...
Naar du importerer ADR-er, blir de opprinnelige ID-ene **nytilordnet** for aa matche prosjektets domeneprefikser. Hver ADRs `domain`-felt bestemmer hvilket prefiks den faar -- for eksempel blir en ADR med `domain: frontend` til `FE-XXX`, mens en med `domain: backend` blir `BE-XXX`. Hvert domene har sin egen teller, saa import av en pakke med blandede domener produserer korrekt prefiksede ID-er uten kollisjoner.

For eksempel, naar du importerer en pakke med tre frontend-ADR-er og to backend-ADR-er inn i et prosjekt som allerede har `FE-001` og `BE-001`, produseres:
...
Dette manifestet lar deg spore opprinnelse -- hvor hver importerte ADR kom fra og naar. Commit det til versjonskontroll sammen med ADR-ene dine.
...
| `--dry-run` | Forhaandsvis endringer uten aa skrive filer |
