...
Når en sjekk feiler, blokkeres committen. Brudd skrives ut til stdout med filstier og linjenumre slik at du kan finne og fikse dem umiddelbart.
...
| `--output json`   | Skriv ut resultater som JSON -- nyttig for å pipe til andre verktøy eller egendefinerte rapporteringsskript                |
| `--adr <id>`      | Sjekk bare regler fra en spesifikk ADR -- nyttig for å isolere en enkelt regel under feilsøking                            |
| `--output github` | Skriv ut GitHub Actions-annotasjoner -- bruk dette i CI-arbeidsflyter i stedet for pre-commit-hooks                        |
