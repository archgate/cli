...
To detaljer er viktige her: opplastingssteget trenger `if: always()`, fordi `archgate check` avslutter med kode 1 ved brudd, og funnene må fortsatt nå Security-fanen nettopp når det finnes funn; og jobben trenger tillatelsen `security-events: write`, ellers avvises opplastingen.
