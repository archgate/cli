...
Pluginet distribueres som en autentisert tarball. CLI-en laster det ned og pakker bare ut katalogene `agents/` og `skills/` i `~/.cursor/`; alt annet i tarballen forkastes. Gamle `archgate-*`-agenter og skill-kataloger fra en tidligere installasjon fjernes først, så lokale endringer i disse filene overlever ikke en reinstallasjon. Ingen CLI-deteksjon er nødvendig -- filene skrives direkte til Cursor-brukerkatalogen.

`afterFileEdit`-hooken hentes ikke fra tarballen. CLI-en skriver den inn i `~/.cursor/hooks.json` selv, og bevarer eventuelle hooks du allerede har der, uten å røre en feilformatert fil.
...
| Fil                                                  | Formål                                                                  |
| ---------------------------------------------------- | ----------------------------------------------------------------------- |
| `~/.cursor/skills/archgate-reviewer/SKILL.md`        | Validerer kodeendringer mot alle prosjektets ADR-er                     |
| `~/.cursor/skills/archgate-lessons-learned/SKILL.md` | Fanger opp læringer og foreslår nye ADR-er når mønstre dukker opp       |
| `~/.cursor/skills/archgate-adr-author/SKILL.md`      | Oppretter og redigerer ADR-er i henhold til prosjektets konvensjoner    |
| `~/.cursor/skills/archgate-onboard/SKILL.md`         | Engangsoppsett: utforsker kodebasen, intervjuer deg og oppretter ADR-er |
| `~/.cursor/skills/archgate-cli-reference/SKILL.md`   | Intern referanse for AI-agenter med Archgate CLI-guiden                 |
...
| `~/.cursor/hooks.json`                               | `afterFileEdit`-hook, flettet inn i eventuelle hooks du allerede har    |

**Prosjektscope**:

| Fil                  | Formål                                                                 |
| -------------------- | ---------------------------------------------------------------------- |
| `.cursor/hooks.json` | `afterFileEdit`-hook som kjører `archgate check` på hver redigerte fil |

Filen `.cursor/hooks.json` er den eneste filen som skrives til prosjekttreet ditt, og `archgate init` skriver den bare når den ikke allerede finnes -- en eksisterende fil røres ikke. Den sørger for at `archgate check` kjøres automatisk etter hver filredigering, og fanger opp ADR-brudd i sanntid.
...
| --------------- | ------------------ | ------------------------------------------------------------------------ |
| `afterFileEdit` | Hver filredigering | Kjører `archgate check` på den redigerte filen for å fange opp ADR-brudd |

Hooken er definert i `.cursor/hooks.json` på prosjektnivå, slik at den fungerer både lokalt og i skybaserte agentmiljøer. Plugininstallasjonen registrerer i tillegg den samme hooken i `~/.cursor/hooks.json` på brukernivå.
...
Når Cursor er den gjenkjente editoren, trenger du ikke oppgi en editor. Bruk `--editor cursor` for å lese Cursors sesjonsutskrifter uansett hva som ble gjenkjent — nyttig når en annen agent kjører inne i Cursor og vinner gjenkjenningen. De to valgene som er relevante for Cursor er:

- `--max-entries <n>` -- Maksimalt antall oppføringer å returnere (standard: 200, nyeste oppføringer). Må være et positivt heltall.
- `--editor <name>` -- Les en annen editors sesjonsutskrifter i stedet for den gjenkjente.

(Et tredje valg, `--root`, gjelder bare opencode.)

Bruk `archgate session-context list` for å oppdage tidligere sesjoner, og `archgate session-context show <session-id>` for å lese en bestemt en.
