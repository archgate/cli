...
**Importerte regler skannes også.** Når du henter inn tredjeparts regler med `archgate adr import`, kjører hver `.rules.ts` gjennom den samme skanneren som førsteparts regler -- **før den skrives til disk**, slik at en importert pakke ikke kan smugle inn kode som din neste `archgate check` ville kjørt.
...
Kommandoen `archgate login` gir tokenene dine til den git credential-helperen du har konfigurert, via `git credential approve`. En helper som bruker operativsystemets legitimasjonsbehandler (macOS Keychain, Windows Credential Manager eller Linux libsecret) holder dem krypterte; gits innebygde `store`-helper skriver dem til `~/.git-credentials` i klartekst, så sjekk din med `git config --global credential.helper`. Tokenet brukes for plugin-installasjon og sendes aldri til tredjeparter utover Archgate plugins-tjenesten.

- Plugin-installasjonskommandoer legger aldri tokenet inn i en URL eller sender det som et kommandolinjeargument: git-baserte installasjoner autentiserer gjennom git credential-helperen din, og API-nedlastinger sender tokenet i en `Authorization`-header. Ingen av delene er synlige i prosesslister.
...
Ikke rapporter sikkerhetssårbarheter gjennom offentlige GitHub-saker. Hvis du oppdager et sikkerhetsproblem i Archgate, rapporter det via [GitHub Security Advisories](https://github.com/archgate/cli/security/advisories/new), inkludert en beskrivelse av sårbarheten, trinn for å reprodusere den, berørte versjoner og eventuell potensiell innvirkning.
