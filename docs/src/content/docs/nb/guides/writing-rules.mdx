...
Noen regler bryr seg ikke om en fils nåværende tilstand, men om **hva som har endret seg**. Et vanlig tilfelle: frafalle et krav (en versjonsheving, en endringsloggoppføring) når en endring bare gjelder dokumentasjon -- den kjørbare strukturen er urørt, og bare kommentarer eller dokstrenger har flyttet seg. `ctx.ast(path, language, { rev: "base" })` parser filen ved den grunnleggende git-revisjonen, slik at du kan sammenligne den mot parsingen av arbeidstreet. Kommentarer og formatering er fraværende fra både ESTree- og Python-`ast`-formene, så en ren kommentarendring gir identiske trær.
    ...
    "bump-required-on-behavior-change": {
      ...
      async check(ctx) {
        ...
        for (const file of changed) {
          // Ingen basis å sammenligne mot (f.eks. en lagt-til fil) -- hopp over.
...
Dette kjøres med `archgate check --base <ref>`; uten en basis er `ctx.changedFiles` tom, og regelen blir en no-op. Fordi basisparsing går gjennom `ctx.ast()` og dens git-tilgang er innebygd i Archgate, kjører regelen aldri `git` eller en tolk selv -- noe [regelsandkassen](/guides/security/) uansett ville blokkert.
