...
- `{ comments: true }` fester en strukturert `comments`-matrise til treet (`typescript`/`javascript`/`python`), slik at kommentarstyringsregler kan jobbe mot kommentar-tokener i stedet for regulære uttrykk.
...
Les en fils kildekode ved den **grunnleggende git-revisjonen** -- fellespunktet (merge base) mellom `--base`-referansen og `HEAD`. Returnerer `null` når ingen basis er løst (ingen `--base`) eller filen ikke fantes ved basisen (en lagt-til fil), slik at en enkelt null-sjekk dekker begge tilfellene. For en strukturell sammenligning, foretrekk `ctx.ast(path, language, { rev: "base" })`.
