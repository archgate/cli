...
- **Skann transitive avhengigheter (standard her)**: `ctx.glob("node_modules/{*,@*/*}/package.json")` går allerede gjennom hver installerte pakke — direkte og transitiv, inkludert scoped pakker via `@*/*`-grenen.
- **Sjekk kun produksjonsavhengigheter**: Les rot-`package.json` med `ctx.readJSON("package.json")` og filtrer `pkgFiles` ned til navnene som finnes i dens `dependencies`-felt hvis du bare vil beskytte bundlet/levert kode, ikke `devDependencies`.
