...
| `--strict`          | Behandle enhver advarsel, og veiledende funn (sammendragsbudsjett, undertrykkelse, utolkede ADR-er), som feil. Se [Streng modus](#streng-modus). |
...
`if: always()` er påkrevd: `archgate check` avslutter med kode 1 når den finner brudd, noe som ellers ville hoppet over opplastingssteget akkurat når det finnes funn å rapportere. Jobben trenger også tillatelsen `security-events: write` for at opplastingen skal lykkes.
...
`--strict` kombinerer to opptrappinger bak ett enkelt valg: ethvert regelbrudd med `"severity": "warning"` mislykker bygget (feltet `warningsExceeded` i JSON-utdataene er `true` og `pass` er `false`), og den mislykker separat bygget når `briefingWarnings`, `suppressionWarnings` eller `unparsedAdrs` ikke er tom — de veiledende diagnostikkene ovenfor som som standard aldri påvirker `pass`. Feltet `strictAdvisoryExceeded` i JSON-utdataene er `true` når sistnevnte tilstand utløste feilen.

Veiledende diagnostikk dekker hele korpuset, ikke bare reglene: den samles inn og håndheves selv når ingen ADR med `rules: true` finnes, så et ADR-korpus med bare prosa mislykkes fortsatt med `--strict` ved et sammendragsbudsjett-overskridelse eller en utolkbar ADR-fil.
