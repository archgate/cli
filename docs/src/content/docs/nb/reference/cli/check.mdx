...
Behandle enhver advarsel og ethvert veiledende funn (sammendragsbudsjett, undertrykkelse, utolkede ADR-er) som en feil:
...
`--strict` behandler enhver advarselslignende diagnostikk som en feil med ett enkelt valg: den oppfører seg som `--max-warnings 0` for regelbaserte advarsler (en eksplisitt `--max-warnings` vinner fortsatt), og den mislykker også bygget når `briefingWarnings`, `suppressionWarnings` eller `unparsedAdrs` ikke er tom — de veiledende diagnostikkene ovenfor som som standard aldri påvirker `pass`. Feltet `strictAdvisoryExceeded` i JSON-utdataene er `true` når sistnevnte tilstand utløste feilen.

`--strict` gjelder også for [`archgate review-context`](/reference/cli/review-context/) og `archgate adr sync`.
...
En eksplisitt `--strict` på kommandolinjen overstyrer alltid konfigurasjonsstandarden.
