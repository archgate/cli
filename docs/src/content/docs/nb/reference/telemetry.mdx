...
- **Ingen personopplysninger**: ingen e-postadresser, passord, hemmeligheter eller IP-adresser. To kvalifiserte unntak: engangshendelsen `project_initialized` kan inkludere `repo_owner` og `repo_name` for repositorier som er bekreftet offentlige av verten sin (se [Repoidentitet](#repoidentitet); private og selvhostede repoer får aldri identitet delt), og krasjrapporter inkluderer den absolutte installasjonsstien til den `archgate`-kjørbare filen, som kan inneholde OS-kontonavnet ditt (se [Feilsporing](#feilsporing-sentry)).
