...
- **`.yml`-/`.yaml`-filer**: hele dokumentet parses som YAML. `frontmatter` er alltid `null`; `content` er den parsede verdien -- cast til forventet type i regelen din, som med `readJSON`. Fordi valget skjer per filendelse, blir `---`-skilletegnene i en flerdokumentstrøm (Kubernetes-manifester, CI-konfigurasjoner) aldri feiltolket som frontmatter.
