---
title: Regler
description: Forstå Archgates TypeScript-regelsystem, lint-steget som gjør Architecture Decision Records om til kjørbare kontroller med bruddrapportering på filnivå.
---
...
Det er denne koblingen som gjør Archgate til lint-steget for AI-arbeid: hver kontroll sporer tilbake til en dokumentert beslutning, og brudd peker på filen og linjen der de oppsto, når reglene oppgir dem.
...
Når `archgate check` kjøres uten `--strict`, betyr exit code 1 at minst ett brudd med alvorlighetsgrad `error` ble funnet. Exit code 0 betyr ingen feil (advarsler og informasjonsmeldinger logges, men blokkerer ikke). Med `--strict` kan advarsler og enkelte rådgivende funn også gi exit code 1 -- se [Konfigurasjon -- `strict`](/reference/configuration/#strict).
