...
Dois detalhes importam aqui: a etapa de upload precisa de `if: always()`, porque `archgate check` sai com código 1 quando há violações e os achados ainda precisam chegar à aba Security justamente quando existem achados; e o job precisa da permissão `security-events: write`, ou o upload é rejeitado.
