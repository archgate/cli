...
Regras são funções TypeScript que fazem lint do seu codebase contra as decisões do time. Elas ficam em arquivos `.rules.ts` complementares ao lado dos arquivos markdown dos ADRs e são executadas quando você roda `archgate check`.
