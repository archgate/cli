...
Regras são checagens TypeScript que fazem lint do seu codebase contra as decisões do time. Elas ficam em arquivos `.rules.ts` complementares ao lado dos arquivos markdown dos ADRs e são executadas quando você roda `archgate check`.
...
O arquivo complementar deve ter o mesmo nome base do arquivo do ADR, e o frontmatter do ADR deve definir `rules: true` -- `archgate check` só carrega arquivos complementares de ADRs que declaram isso. Um ADR com `rules: true` e sem arquivo complementar é reportado como um erro de regra (código de saída 2).
...
Lê e faz o parse de um arquivo JSON. Retorna `unknown` para um caminho arbitrário -- faça cast para o formato esperado. O caminho literal `"package.json"` tem uma sobrecarga dedicada que retorna um `PackageJson` tipado, então nenhum cast é necessário nesse caso.

```typescript
// Tipado pela sobrecarga -- pkg.dependencies é Record<string, string> | undefined
const pkg = await ctx.readJSON("package.json");

// Qualquer outro caminho retorna unknown
...
- `comments: true` anexa um array `comments` estruturado à árvore (todas as quatro linguagens), para que regras de governança de comentários trabalhem com tokens de comentário estruturados em vez de regex.
...
Lê o código-fonte de um arquivo na **revisão base do git** -- a base de merge entre a ref base e `HEAD`. `archgate check` resolve essa ref por conta própria (`--base <ref>`, depois a branch base configurada, depois auto-detecção), então uma base normalmente está disponível sem passar nenhuma flag. Retorna `null` quando nenhuma base pôde ser resolvida (uma execução com `--staged`, um projeto que não é um repositório git, nenhuma branch base detectável, ou históricos não relacionados) ou quando o arquivo não existia na base (um arquivo adicionado), então uma única verificação de null cobre ambos os casos. Para uma comparação estrutural, prefira `ctx.ast()` com `rev: "base"`.
...
| `endLine`   | `number` | Não         | Linha final do intervalo com o problema (`--output json` e SARIF)  |
| `endColumn` | `number` | Não         | Coluna final do intervalo com o problema (`--output json` apenas)  |
| `fix`       | `string` | Não         | Correção sugerida (`--output json`; no console só com `--verbose`) |
...
Chamar `ctx.ast()` com `comments: true` anexa um array `comments` à árvore, para que regras de política de comentários trabalhem com tokens estruturados (`type`, `value`, `loc`) em vez de regex linha a linha. As posições dos comentários são precisas em relação ao código-fonte original mesmo para TypeScript. Suportado para todas as quatro linguagens -- para `ruby`, onde a árvore é um array `Ripper.sexp`, o array `comments` fica pendurado nesse array como uma propriedade não indexada.
...
A severidade de um resultado é decidida pelo método `ctx.report.*` que o produziu -- não há um campo de severidade separado para manter sincronizado:

| Método de report         | Severidade | Código de saída | Comportamento                               |
| ------------------------ | ---------- | --------------- | ------------------------------------------- |
| `ctx.report.violation()` | `error`    | 1               | Bloqueia CI, deve ser corrigido             |
| `ctx.report.warning()`   | `warning`  | 0               | Registrado, mas não bloqueia                |
| `ctx.report.info()`      | `info`     | 0               | Informacional, registrado para visibilidade |

Então uma regra que nunca deve bloquear o CI reporta via `warning()`:
...
export default {
  rules: {
    "my-rule": {
      description: "...",
      async check(ctx) {
        // Reportado como warning -- a verificação ainda sai com código 0
...
Uma única regra pode misturar os três, reportando alguns resultados como erros e outros como warnings ou notas.

:::caution
`RuleConfig` também aceita um campo `severity` opcional. Ele é validado quando o arquivo de regras é carregado, mas **não** muda a severidade de nada que a regra reporte -- `ctx.report.violation()` é sempre um erro, mesmo em uma regra declarada com `severity: "warning"`. Escolha o método de report, não o campo.
:::

Sob `archgate check --strict`, qualquer resultado com severidade `warning` falha a execução (código de saída 1) em vez de apenas ser registrado. Resultados `info` nunca afetam o código de saída.
...
O campo `fix` é uma string opcional que descreve qual ação tomar para resolver o problema. Correções não são aplicadas automaticamente -- são orientações.
...
O reporter de console imprime o fix abaixo da violação **apenas com `--verbose`**; caso contrário, mostra somente a mensagem e `file:line`:
...
`--output json` sempre carrega a string `fix`, então agentes e integrações de editor a veem independentemente de `--verbose`. Os reporters do GitHub Actions e SARIF emitem apenas a mensagem -- coloque qualquer coisa que um revisor precise ler em `message`, não em `fix`.
...
Arquivos **Markdown** (`.md`, `.mdx`) também aceitam a forma de comentário HTML, que permanece invisível na página renderizada — `#` viraria um título e `//` apareceria como texto do corpo:

```md
<!-- archgate-ignore ARCH-021/no-escaped-backtick a crase é citada de uma transcrição de shell -->

<!-- archgate-ignore-file ARCH-021/ai-writing-signs trecho literal de uma RFC externa -->
```

O comentário deve ocupar a linha inteira, e diretivas dentro de blocos de código cercados são ignoradas para que exemplos documentados permaneçam inertes.
