...
O plugin para Cursor está atualmente em beta. Execute `archgate login` para entrar.
:::
...
O plugin é distribuído como um tarball autenticado. A CLI faz o download e extrai apenas os diretórios `agents/` e `skills/` em `~/.cursor/`; qualquer outro conteúdo do tarball é descartado. Agentes `archgate-*` e diretórios de skills obsoletos de uma instalação anterior são removidos primeiro, então edições locais nesses arquivos não sobrevivem a uma reinstalação. Nenhuma detecção de CLI é necessária -- os arquivos são gravados diretamente no diretório de usuário do Cursor.

O hook `afterFileEdit` não vem do tarball. A CLI o grava em `~/.cursor/hooks.json` diretamente, preservando quaisquer hooks que você já tenha ali e deixando um arquivo malformado intocado.
...
| Arquivo                                              | Propósito                                                                     |
| ---------------------------------------------------- | ----------------------------------------------------------------------------- |
| `~/.cursor/skills/archgate-reviewer/SKILL.md`        | Valida alterações de código contra todos os ADRs do projeto                   |
| `~/.cursor/skills/archgate-lessons-learned/SKILL.md` | Captura aprendizados e propõe novos ADRs quando padrões emergem               |
| `~/.cursor/skills/archgate-adr-author/SKILL.md`      | Cria e edita ADRs seguindo as convenções do projeto                           |
| `~/.cursor/skills/archgate-onboard/SKILL.md`         | Configuração única: explora o codebase, entrevista você e cria ADRs           |
| `~/.cursor/skills/archgate-cli-reference/SKILL.md`   | Referência interna para agentes de IA com o guia CLI do Archgate              |
...
| `~/.cursor/hooks.json`                               | Hook `afterFileEdit`, mesclado em quaisquer hooks que você já tenha           |

**Escopo do projeto**:

| Arquivo              | Propósito                                                                 |
| -------------------- | ------------------------------------------------------------------------- |
| `.cursor/hooks.json` | Hook `afterFileEdit` que executa `archgate check` em cada arquivo editado |

O arquivo `.cursor/hooks.json` é o único arquivo gravado na árvore do seu projeto, e o `archgate init` só o grava quando ele ainda não existe -- um arquivo existente é deixado intacto. Ele garante que `archgate check` seja executado automaticamente após cada edição de arquivo, detectando violações de ADR em tempo real.
...
| Hook            | Gatilho                | Ação                                                                       |
| --------------- | ---------------------- | -------------------------------------------------------------------------- |
| `afterFileEdit` | Cada edição de arquivo | Executa `archgate check` no arquivo editado para detectar violações de ADR |

O hook é definido em `.cursor/hooks.json` no nível do projeto, então funciona tanto localmente quanto em ambientes de agentes na nuvem. A instalação do plugin adicionalmente registra o mesmo hook no seu `~/.cursor/hooks.json` de nível de usuário.
...
O comando `archgate session-context` lê transcrições de sessão do agente Cursor do disco. Isso permite que skills acessem o histórico da conversa atual, o que é útil para recuperar contexto que pode ter sido compactado ou truncado.

Quando o Cursor é o editor detectado, não é preciso informar um editor. Use `--editor cursor` para ler as transcrições do Cursor independentemente do que foi detectado — útil quando outro agente executa dentro do Cursor e vence a detecção. As duas opções relevantes para o Cursor são:

- `--max-entries <n>` -- Número máximo de entradas a retornar (padrão: 200, entradas mais recentes). Deve ser um número inteiro positivo.
- `--editor <name>` -- Lê as transcrições de outro editor em vez do detectado.

(Uma terceira opção, `--root`, se aplica apenas ao opencode.)

Use `archgate session-context list` para descobrir sessões anteriores e `archgate session-context show <session-id>` para ler uma sessão específica.
