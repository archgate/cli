...
**URL do marketplace:** Abra o JSON de configurações de usuário do VS Code (`Ctrl+Shift+P` → "Preferences: Open User Settings (JSON)") e adicione a entrada `chat.plugins.marketplaces` mostrada acima. Você pode encontrar sua URL autenticada executando `archgate plugin url vscode`.
...
Substitua `<token>` pelo seu token archgate (obtenha-o via `archgate plugin url vscode`).

## O que o plugin oferece

O plugin adiciona um agente e skills baseadas em papéis ao VS Code. O agente orquestra o fluxo de guardrails, invocando skills conforme necessário.
