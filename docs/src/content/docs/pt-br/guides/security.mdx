...
**Regras importadas também são escaneadas.** Quando você importa regras de terceiros com `archgate adr import`, cada `.rules.ts` passa pelo mesmo scanner usado para regras próprias -- **antes de ser gravado em disco**, de modo que um pacote importado não consiga contrabandear código que seu próximo `archgate check` executaria.
...
O comando `archgate login` entrega seus tokens ao credential helper do git que você configurou, via `git credential approve`. Um helper apoiado no gerenciador de credenciais do sistema operacional (macOS Keychain, Windows Credential Manager ou libsecret no Linux) os mantém criptografados; o helper `store` embutido do git os grava em `~/.git-credentials` em texto puro, então confira o seu com `git config --global credential.helper`. O token é usado para instalação de plugins e nunca é enviado a terceiros além do serviço de plugins do Archgate.

- Comandos de instalação de plugins nunca embutem o token em uma URL nem o passam como argumento de linha de comando: instalações baseadas em git se autenticam através do seu credential helper do git, e downloads via API enviam o token em um cabeçalho `Authorization`. Nenhum dos dois fica visível em listagens de processos.
...
Por favor, não reporte vulnerabilidades de segurança através de issues públicas no GitHub. Se você descobrir um problema de segurança no Archgate, reporte-o via [GitHub Security Advisories](https://github.com/archgate/cli/security/advisories/new), incluindo uma descrição da vulnerabilidade, passos para reproduzi-la, versões afetadas e qualquer impacto potencial.
