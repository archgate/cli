...
- Comandos de instalação de plugins nunca embutem o token em uma URL nem o passam como argumento de linha de comando: instalações baseadas em git se autenticam através do gerenciador de credenciais do SO, e downloads via API enviam o token em um cabeçalho `Authorization`. Nenhum dos dois fica visível em listagens de processos.
