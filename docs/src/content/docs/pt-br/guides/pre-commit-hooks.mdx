...
Quando uma verificação falha, o commit é bloqueado. As violações são impressas em stdout com caminhos de arquivo e números de linha para que você possa localizá-las e corrigi-las imediatamente.
...
| `--output json`   | Exibe resultados como JSON -- útil para encaminhar a outras ferramentas ou scripts de relatório personalizados                               |
| `--adr <id>`      | Verifica apenas regras de um ADR específico -- útil para isolar uma única regra durante o debug                                              |
| `--output github` | Exibe anotações do GitHub Actions -- use em workflows de CI ao invés de hooks de pre-commit                                                  |
