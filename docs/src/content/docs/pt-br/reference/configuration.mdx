...
Padrão do projeto para `--strict`, suportado por `archgate check`, `archgate review-context` e `archgate adr sync`. `--strict` transforma diagnósticos consultivos em falhas -- veja [`archgate check` -- Modo estrito](/reference/cli/check/#modo-estrito) para o que exatamente cada comando reprova.

| Tipo      | Padrão  | Descrição                                            |
| --------- | ------- | ---------------------------------------------------- |
| `boolean` | `false` | Habilita o modo estrito para este projeto por padrão |

A ordem de resolução é: flag de linha de comando, depois este valor de config, depois `false`. Não existe uma flag `--no-strict`, então a flag de linha de comando só pode _ativar_ o modo estrito sobre um valor de config ausente ou `false` -- ela não pode desativar um `strict: true` já configurado.
