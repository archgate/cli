...
- Repositórios privados (a consulta retorna 404 ou `private: true`)
...
**Opt-out** (para repos públicos onde você ainda prefere ficar anônimo):

```bash
# Flag por invocação
archgate init --no-share-repo-identity

# Ou via variável de ambiente — aplica-se a toda invocação
export ARCHGATE_SHARE_REPO_IDENTITY=0
archgate init
```

Valores de opt-out aceitos: `0`, `false`, `no`, `off` (sem diferenciação de maiúsculas/minúsculas).
