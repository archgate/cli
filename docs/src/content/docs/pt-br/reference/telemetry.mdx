...
- **Nome do comando** e **quais flags foram usadas** (ex: `check --output json`; a presença da flag mais o formato selecionado em `--output` — uma escolha fixa entre `console`/`json`/`github`/`sarif` — nunca valores livres de flags)
...
- **Localização aproximada**: país e região, resolvidos no servidor a partir do seu IP (descartado em seguida; veja [Anonimização de IP](#anonimização-de-ip))
...
- **`check`**: contagens agregadas de regras (total, aprovadas, reprovadas, avisos, erros), formato de saída usado, se filtros foram aplicados, arquivos analisados, duração de carregamento, duração da verificação, e linguagens/runtimes/frameworks detectados no projeto (comparados com uma lista fixa de sinais conhecidos de dependências e arquivos de configuração, ex.: `typescript`, `bun`, `nextjs`). Sem caminhos de arquivo ou conteúdo de violação
...
**Não quer o evento de forma alguma?** Desative a telemetria por completo. O evento `project_initialized` é suprimido quando você desativa a telemetria:
...
- **Tipo do erro, mensagem e stack trace**, incluindo os caminhos de arquivo do local da falha. Para o binário compilado (a instalação padrão), esses são caminhos internos do bundle do Bun (ex.: `/$bunfs/root/cli.js`), não caminhos do seu sistema de arquivos. Para uma instalação `local` (dependência de desenvolvimento) ou `global-pm`, os frames do stack podem incluir o caminho absoluto real do arquivo da CLI instalado na sua máquina
- **Caminho de instalação**: o caminho absoluto do executável `archgate` instalado ou do script de entrada, enviado como uma tag do Sentry em cada relatório de falha
- **Contexto de execução**: SO, arquitetura, versão do Bun, versão do Archgate, método de instalação
- **ID de instalação anônimo** (mesmo UUID aleatório da análise)

## O que NÃO coletamos

- **Nenhuma informação pessoal**: nenhum email, senha, segredo ou endereço IP. Duas exceções qualificadas: o evento único `project_initialized` pode incluir a URL remota, `repo_owner` e `repo_name` para repositórios confirmados como públicos pelo host (veja [Identidade do repositório](#identidade-do-repositório); repositórios privados e auto-hospedados nunca têm identidade compartilhada), e relatórios de falha incluem o caminho absoluto de instalação do executável `archgate`, que pode conter o nome da sua conta no sistema operacional (veja [Rastreamento de erros](#rastreamento-de-erros-sentry)).
- **Nenhum conteúdo de arquivo**: nenhum conteúdo de ADR ou código-fonte. Relatórios de erro (Sentry) incluem os caminhos de arquivo do local da falha no stack trace e o caminho absoluto de instalação do executável `archgate` (veja [Rastreamento de erros](#rastreamento-de-erros-sentry) para saber exatamente como isso aparece em cada método de instalação), mas nunca conteúdos de arquivo.
- **Nenhum contexto de IA**: nada de interações com agentes, prompts ou conteúdo gerado por IA
- **Nenhum valor livre de flag**: a única exceção é o formato selecionado em `--output` (um enum fixo: `console`/`json`/`github`/`sarif`); nunca registramos valores fornecidos pelo usuário nem o conteúdo da saída
- **Nenhuma atividade de rede**: nenhuma chave de API ou token, e nenhuma URL — com uma exceção: a URL remota de um repositório confirmado como público pelo host, no evento único `project_initialized` (veja [Identidade do repositório](#identidade-do-repositório))
...
3. O endereço IP é então **descartado**. Ele nunca é armazenado no PostHog
