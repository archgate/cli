...
- **Contexto do repositório** (não identificável): se o diretório atual é um repositório git, o bucket do host (`github` / `gitlab` / `bitbucket` / `other`), um **`repo_id` com hash** (SHA-256 da URL remota normalizada, truncado em 16 caracteres hex — não reversível) e o nome do branch padrão
...
Por padrão, o Archgate envia um `repo_id` **com hash** em todo evento para que possamos contar repositórios distintos usando a CLI sem aprender seus nomes. A URL remota bruta, o owner e o nome do repositório **não** são incluídos no fluxo comum de eventos.

Se você quiser nos ajudar a ver exatamente quais repositórios públicos estão adotando o Archgate, você pode optar pelo compartilhamento. Quando habilitado, o evento `project_initialized` único (emitido por `archgate init`) leva `remote_url`, `repo_owner` e `repo_name`. Nenhum outro evento é afetado.
...
Valores aceitos: `1`, `true`, `yes`, `on` (sem diferenciação de maiúsculas/minúsculas).
...
- **Nenhuma informação pessoal**: nenhum nome de usuário, email ou endereço IP. Owners e nomes de repositórios do GitHub só são incluídos quando você faz opt-in via `--share-repo-identity` ou `ARCHGATE_SHARE_REPO_IDENTITY=1` (veja [Identidade do repositório](#identidade-do-repositório))
