...
Com `{ comments: true }`, a árvore retornada carrega um array `comments` -- dados estruturados de comentário para regras de governança de comentários, no lugar de regex linha a linha. Suportado para `typescript`, `javascript` e `python`; solicitar isso para `ruby` lança uma exceção.
...
Quando parseado com [`{ comments: true }`](#ast), o nó raiz também carrega um array `comments: CommentToken[]` (apenas `typescript`, `javascript` e `python`). Ausente nos demais casos.
