...
- **`--output sarif`** carrega `message`, `file`, `line` e `endLine` (como uma `region` do SARIF). Colunas e `fix` são omitidos -- o SARIF não tem `startColumn` para parear com `endColumn`.
