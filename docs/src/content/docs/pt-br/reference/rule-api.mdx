...
- **Arquivos `.yml` / `.yaml`**: o documento inteiro é parseado como YAML. `frontmatter` é sempre `null`; `content` é o valor parseado -- faça o cast para o tipo esperado na sua regra, como com `readJSON`. Como a escolha é por extensão, os separadores `---` de um stream multi-documento (manifestos Kubernetes, configurações de CI) nunca são interpretados erroneamente como frontmatter.
