...
| `domain` | `string`   | Sim         | Nome de domínio registrado em kebab-case minúsculo. Integrados: `backend`, `frontend`, `data`, `architecture`, `general`. Domínios personalizados são aqueles registrados via [`archgate adr domain add`](/pt-br/reference/cli/adr/#archgate-adr-domain). |
...
Projetos podem estender o conjunto integrado com domínios personalizados via [`archgate adr domain add`](/pt-br/reference/cli/adr/#archgate-adr-domain). Os mapeamentos de domínio → prefixo personalizados ficam em `.archgate/config.json` e são mesclados com os integrados no momento da leitura.
...
Projetos podem registrar mapeamentos adicionais de domínio → prefixo via [`archgate adr domain add`](/pt-br/reference/cli/adr/#archgate-adr-domain). Uma vez registrados, domínios personalizados se comportam como integrados: `archgate adr create --domain <nome>` gera IDs automaticamente usando o prefixo associado, e ADRs com domínios personalizados são analisados sem erros.

Veja a [página de conceitos de Domínios](/pt-br/concepts/domains/) para orientação sobre quando introduzir um domínio personalizado.
