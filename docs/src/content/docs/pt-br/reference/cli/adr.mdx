...
Exibe todos os domínios mesclados (integrados + personalizados) e seus prefixos:
...
### archgate adr domain add

Registra um domínio personalizado:
...
- `<name>`: kebab-case em minúsculas, 2–32 caracteres, deve começar com uma letra minúscula (ex.: `security`, `ml-ops`)
- `<prefix>`: letras maiúsculas, dígitos ou underscores, 2–10 caracteres, deve começar com uma letra maiúscula (ex.: `SEC`, `MLOPS`)
...
### archgate adr domain remove

Remove um domínio personalizado (integrados não podem ser removidos):

```bash
archgate adr domain remove <name>
```
...
O comando clona a fonte, lê os arquivos ADR, remapeia os IDs para a sequência do projeto local e os grava em `.archgate/adrs/`. As importações são rastreadas em `.archgate/imports.json` para que possam ser verificadas posteriormente via [`archgate adr sync`](#archgate-adr-sync).

### Argumentos

| Argumento     | Descrição                                                                                                  |
| ------------- | ---------------------------------------------------------------------------------------------------------- |
| `<source...>` | Caminho(s) do registro, `org/repo/path` ou URL(s) git. Obrigatório mesmo com `--list`, que ignora o valor. |
...
Listar ADRs importados anteriormente (o argumento `<source...>` ainda é exigido pelo parser, mas seu valor é ignorado com `--list`):

```bash
archgate adr import archgate/packs/typescript --list
```
...
| `--json`   | Saída como JSON                                                                                                                                  |
| `--strict` | Falha (código 1) quando a sincronização encontrar erros (fonte de importação não resolvida, clone falhou, arquivo local/upstream do ADR ausente) |

`--strict` é resolvido da mesma forma que [`archgate check --strict`](/reference/cli/check/#modo-estrito): flag de linha de comando, depois uma chave `strict: boolean` em `.archgate/config.json`, depois desligado. Não existe uma flag `--no-strict` -- a flag de linha de comando só pode ativar o modo estrito, não sobrepor um `strict: true` já configurado.
...
Falhar o build se alguma importação não puder ser resolvida, clonada ou associada a um ADR local:
