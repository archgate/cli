...
Elas são **consultivas e nunca afetam `pass`**. Um ADR cuja seção não pode encolher sem perder uma cláusula normativa — uma lista enumerada de identificadores, uma barreira de proteção ordenada, uma isenção — deve mesmo exceder o limite. Quando isso ocorrer, registre o motivo na seção Compliance do próprio ADR e organize a seção para que o conteúdo mais normativo venha antes do corte, já que o truncamento sempre remove o final.

Para reduzir um excesso: mova justificativas para Context ou Consequences, que nunca entram no resumo e portanto nunca têm limite; remova narrativa histórica; e mescle regras duplicadas.
