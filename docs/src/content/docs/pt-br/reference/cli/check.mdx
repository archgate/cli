...
| `--strict`          | Trata qualquer aviso, e achados consultivos (orçamento do resumo, supressão, ADRs não analisados), como falhas. Veja [Modo estrito](#modo-estrito). |
...
O `if: always()` é obrigatório: `archgate check` sai com código 1 quando encontra violações, o que pularia a etapa de upload exatamente quando há achados a reportar. O job também precisa da permissão `security-events: write` para o upload funcionar.
...
Os diagnósticos consultivos abrangem todo o corpus, não apenas as regras: eles são coletados e aplicados mesmo quando nenhum ADR com `rules: true` existe, então um corpus de ADRs só de prosa ainda falha com `--strict` diante de um estouro de orçamento do resumo ou de um arquivo de ADR não analisável.
