...
| 3          | `.archgate/config.json` `baseBranch` | `git diff <resolved-ref>...HEAD` + árvore de trabalho      |
| 4          | Auto-detecção do git                 | `git diff <detected-ref>...HEAD` + árvore de trabalho      |
| 5          | Detecção falha                       | Vazio (modo de varredura completa)                         |

A auto-detecção tenta `origin/HEAD`, depois `origin/main`, `origin/master`, `main` local e `master` local. Para definir um padrão do projeto, adicione `baseBranch` ao `.archgate/config.json`:
