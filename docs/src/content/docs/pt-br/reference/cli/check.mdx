...
Tratar qualquer aviso e qualquer achado consultivo (orçamento do resumo, supressão, ADRs não analisados) como falha:
...
`--strict` trata todo diagnóstico em forma de aviso como falha em uma única flag: ele se comporta como `--max-warnings 0` para avisos de severidade de regra (um `--max-warnings` explícito ainda prevalece), e também falha a build quando `briefingWarnings`, `suppressionWarnings` ou `unparsedAdrs` não estiver vazio — os diagnósticos consultivos acima que, por padrão, nunca afetam `pass`. O campo `strictAdvisoryExceeded` da saída JSON é `true` quando essa última condição causou a falha.

`--strict` também se aplica a [`archgate review-context`](/reference/cli/review-context/) e `archgate adr sync`.
...
Um `--strict` explícito na linha de comando sempre sobrepõe o padrão do config.
