...
- **GitHub Copilot:** Declara o marketplace e o plugin Archgate no `settings.json` do Copilot (`~/.copilot/` por padrão, respeitando `COPILOT_HOME`), arquivo lido tanto pelo Copilot CLI quanto pelo aplicativo desktop do Copilot. Quando o CLI `copilot` está no PATH, o plugin também é instalado imediatamente; caso contrário, o Copilot o instala automaticamente na próxima inicialização. Exibe comandos manuais quando nenhum CLI `copilot` ou diretório de configurações do Copilot é detectado.
- **Cursor:** Faz o download de um tarball autenticado e extrai skills e agentes em `~/.cursor/`; também mescla uma entrada archgate em `~/.cursor/hooks.json` (gravado localmente pela CLI, e não parte do bundle baixado, preservando quaisquer hooks pré-existentes). Nenhuma detecção de CLI é necessária. Os arquivos são gravados diretamente no diretório de usuário do Cursor.
- **VS Code:** Instala a extensão VS Code (`.vsix`) via CLI `code` se disponível; exibe instruções manuais caso contrário.
- **opencode:** Detecta o opencode pela CLI `opencode` no PATH ou (se a CLI estiver ausente) pela presença do diretório de configuração compartilhado do opencode no escopo do usuário, também usado pelo aplicativo Desktop. Pula a instalação com uma mensagem clara quando nenhum dos dois é encontrado. Quando detectado, baixa um tarball autenticado de arquivos de agente e o extrai no diretório de agentes opencode do escopo do usuário. `archgate plugin url --editor opencode` exibe "N/A" -- opencode não tem URL de marketplace. Veja o [guia de integração com opencode](/guides/opencode-integration/) para detalhes.
