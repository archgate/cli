...
**opencode:** A CLI baixa um tarball autenticado de arquivos de agente do serviço de plugins do Archgate e o extrai no diretório de agentes opencode do escopo do usuário (`~/.config/opencode/agents/` em POSIX, `%APPDATA%\opencode\agents\` no Windows). Nenhum arquivo é gravado na árvore do projeto. Veja o [guia de integração com opencode](/guides/opencode-integration/) para detalhes.
