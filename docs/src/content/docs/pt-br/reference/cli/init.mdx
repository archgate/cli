...
Cria o diretório `.archgate/` com um ADR de exemplo e um diretório de regras do linter. Opcionalmente configura a integração com o editor para fluxos de trabalho com agentes de IA e instala o plugin de editor do Archgate.
...
**opencode:** Requer que o opencode seja detectado -- ou a CLI `opencode` no seu PATH, ou a existência do diretório de configuração do opencode no escopo do usuário (o app Desktop não traz binário de CLI). Se nenhum dos dois for encontrado, a instalação é pulada e uma mensagem pede que você instale o opencode primeiro. Quando detectado, a CLI baixa um tarball autenticado do serviço de plugins do Archgate e extrai seus diretórios `agents/` e `skills/` na raiz de configuração do opencode no escopo do usuário (`$XDG_CONFIG_HOME/opencode/`, com fallback para `$HOME/.config/opencode/` em qualquer plataforma, inclusive Windows; o opencode usa paths XDG via `xdg-basedir` e não lê `%APPDATA%`), e define o agente padrão em `opencode.json` quando nenhum está configurado. Nenhum arquivo é gravado na árvore do projeto. Veja o [guia de integração com opencode](/guides/opencode-integration/) para detalhes.
