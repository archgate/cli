...
O CLI detecta automaticamente os editores instalados (Claude Code, Cursor, VS Code, GitHub Copilot, opencode) e executa a instalação do plugin para cada editor selecionado. Falhas na atualização de plugins são reportadas, mas não afetam o código de saída -- a atualização do CLI em si já está completa.
