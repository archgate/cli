...
| `ctx.changedFiles` | `string[]` | Arquivos alterados no git (auto-detectado a partir do diff da branch, ou via `--staged`/`--base`)      |
...
- **Arquivos alterados auto-detectados** -- `ctx.changedFiles` é automaticamente preenchido com o diff da branch contra a branch base (ex: `main`). Use `--staged` para hooks de pre-commit (apenas arquivos staged) ou `--base <ref>` para uma base explícita. Isso permite que regras de dependência entre arquivos funcionem localmente, não apenas no CI.
