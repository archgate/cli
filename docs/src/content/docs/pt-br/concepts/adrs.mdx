...
O documento é um arquivo Markdown com frontmatter YAML armazenado em `.archgate/adrs/` por padrão. Ele descreve a decisão em linguagem simples: qual problema resolve, quais alternativas foram consideradas, o que a equipe decidiu, e quais consequências seguem.

:::tip[Diretório personalizado de ADRs]
ADRs podem ser armazenados em um diretório diferente (ex: `docs/adrs/`) configurando o campo `paths.adrs` em `.archgate/config.json`. Veja a [referência de Configuração](/reference/configuration/#diretorio-personalizado-de-adrs) para detalhes.
:::
...
| `domain` | string       | Sim         | Nome de domínio registrado. Integrados: `backend`, `frontend`, `data`, `architecture`, `general`. [Domínios personalizados](/concepts/domains/#domínios-personalizados) podem ser adicionados via `archgate adr domain add`. |
