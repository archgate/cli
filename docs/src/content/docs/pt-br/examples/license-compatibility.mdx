...
- **Escanear transitivas (já é o padrão aqui)**: `ctx.glob("node_modules/{*,@*/*}/package.json")` já percorre todo pacote instalado — direto e transitivo, incluindo pacotes escopados via o branch `@*/*`.
- **Verificar apenas deps de produção**: Leia o `package.json` raiz com `ctx.readJSON("package.json")` e filtre `pkgFiles` para os nomes presentes no campo `dependencies` dele, caso você só queira bloquear código empacotado/distribuído, e não `devDependencies`.
