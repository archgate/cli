...
- **Escanear transitivas**: Use `ctx.glob("node_modules/*/package.json")` e `ctx.glob("node_modules/@*/*/package.json")` para escanear todos os pacotes instalados recursivamente, não apenas dependências diretas.
