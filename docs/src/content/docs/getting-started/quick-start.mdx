...
Want AI agents that automatically read your ADRs before coding? Run `archgate login` to sign up and authenticate directly from the CLI, then run `archgate init --install-plugin` to set up the plugin.
:::
