...
- **Archgate rule** `ARCH-012/async-action-error-boundary`: Walks the AST (`ctx.ast`) of each async command action and enforces (1) the action body contains a top-level try-catch, and (2) no top-level awaited statement sits _outside_ that try block — escaped awaits are the statements whose rejections bypass the boundary into `main().catch()`. Awaits of the sanctioned exit paths (`exitWith`, `handleCommandError`) are exempt as approved terminal paths, so early-return guards remain allowed. `handleCommandError` does reject for `ExitPromptError`, and that propagation is deliberate: it lets `main().catch()` exit 130 on Ctrl+C without logging the cancellation or capturing it to Sentry. Severity: `warning` (some commands may have valid reasons for alternative patterns). **Remaining limitation:** synchronous statements outside the try are not flagged — sync throws from prelude code (e.g. argument validation) still escape; keep preludes trivial or move them inside the try.
