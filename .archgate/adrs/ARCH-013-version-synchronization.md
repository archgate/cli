...
9. `shims/winget/manifests/*.yaml` — winget `PackageVersion`, plus the `InstallerUrl` and `ReleaseNotesUrl` that embed the version
...
`shims/winget/` participates in version synchronization only. Its manifests carry `PackageVersion` plus two URLs that embed the version, all bumped by the hook and covered by `shim-version-sync`. No package is built from that directory — winget installs an executable cross-compiled from `shims/go` ([ARCH-017](./ARCH-017-multi-ecosystem-distribution.md)) from a release URL — so it has no registry page rendering a README and no artifact for a `LICENSE.md` to ship inside. Its absence from those two rule lists is the correct state, not drift.
