...
**TypeScript version:** `typescript` MUST be an explicit, exact-pinned `devDependency` in `package.json` (currently `7.0.2`), not left to resolve transitively from `peerDependencies` alone — see Key Definitions: Version Pairing for how this differs from the `peerDependencies.typescript` range consumers of this CLI's plugin/rule-authoring surface use.
