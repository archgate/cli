...
- **Restore environment variables with `restoreEnv(key, original)` from `tests/test-utils.ts`** — it deletes the key when the captured original was `undefined` and assigns otherwise. Required for every `const original = Bun.env.X` / `process.env.X` override, including the `NODE_ENV`/`GIT_CONFIG_GLOBAL`/`HOME` overrides mandated elsewhere in this ADR. `Bun.env` and `process.env` are the same underlying store, so the helper works regardless of which accessor the file uses.
...
- **Don't restore an environment variable with a bare `Bun.env.X = original` / `process.env.X = original` assignment** — `env.X = undefined` assigns the literal string `"undefined"` and leaves the key present rather than unsetting it, so the idiomatic-looking capture-and-restore silently leaks `X="undefined"` whenever the variable was unset to begin with (the normal case on Windows for `HOME` and `GIT_CONFIG_GLOBAL`). Bun shares one process across test files, so the bogus value escapes into every later test file and into every subprocess they spawn — the symptom is a test that passes in isolation and fails in the full suite. Use `restoreEnv()` (see Do's above). Guarding only some variables is the common failure: five test files restored `ARCHGATE_TELEMETRY` correctly with an explicit `undefined` check while leaving `HOME` bare.
...
- **oxlint plugin** `test-isolation/no-bare-env-restore` (`lint/no-bare-env-restore.ts`): A custom oxlint JS plugin enabled for `tests/**/*.test.ts` that fails the build for any `Bun.env.NAME = <identifier>` or `process.env.NAME = <identifier>` assignment where that identifier was itself captured from an env read earlier in the same file (e.g. `const originalHome = Bun.env.HOME`). Tracking the capture — rather than matching a naming convention such as `original*` — is what separates a restore from an override: both are spelled `Bun.env.HOME = <identifier>`, so `Bun.env.HOME = tempDir` is deliberately left alone, and the real call sites named their variables `savedHome`, `savedDistro`, `savedAppData` and `origCI` as often as `original*`. Computed access (`Bun.env[key]`) is also left alone, being the shape of the `restoreEnv` helper itself rather than the hand-rolled idiom. Runs as part of `bun run lint` (and therefore `bun run validate` and CI). It found three unguarded restores that two rounds of manual grep review had missed.
