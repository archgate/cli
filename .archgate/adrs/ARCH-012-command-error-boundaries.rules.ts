 ...
 * only to decide whether walk()/containsDirectAwait() should descend into a
 * value at all. Keeps descending into every plain object/array, not just
 * ones that carry a `type` field (e.g. `loc`), matching the walk's
 * original behavior.
 */
