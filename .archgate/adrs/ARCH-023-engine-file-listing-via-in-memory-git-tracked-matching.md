...
4. **Sandbox parity** — The rule sandbox contract ([ARCH-022](./ARCH-022-ast-aware-rule-context.md)) MUST hold on both paths: the pattern **and every brace-expanded alternative** pass `safeGlob` validation (no `..`, no absolute paths) before matching or scanning. Brace expansion can surface absolute alternatives hidden inside a group (e.g. `{/etc/passwd,src/a.ts}`).
